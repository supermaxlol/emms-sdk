#!/usr/bin/env python3
"""EMMS v0.4.0 — Claude Model Comparison: Identity Adoption Tests

Tests Claude Opus 4.6, Sonnet 4.5, and Haiku 4.5 to see how model
capability and size affect identity adoption and guardrail resistance.

Hypothesis: Larger models may have STRONGER guardrails (more RLHF training)
but also better instruction following (system prompt compliance).
"""
...
async def run_test(provider, provider_name: str, test_name: str, test_config: dict, emms_context: str):
    results = {}
...
async def main():
    ...
    print("=" * 70)
    print("  EMMS v0.4.0 — Claude Model Comparison: Identity Adoption")
    ...
    for name, model_id in CLAUDE_MODELS:
        try:
            p = ClaudeProvider(api_key=api_key, model=model_id)
            test = await p.generate("Say OK", max_tokens=10)
            print(f"  {name} ({model_id}): READY")
            providers.append((name, p))
        except Exception as e:
            err = str(e)[:80]
            print(f"  {name}: UNAVAILABLE ({err})")

    # Also add local Ollama gemma3n for comparison
    try:
        ...
        print(f"  Ollama-gemma3n (local): READY")
    ...
    print(f"\n  Testing {len(providers)} providers x {len(IDENTITY_TESTS)} tests x 3 strategies = {len(providers) * len(IDENTITY_TESTS) * 3} trials\n")
    ...
    # ══════════════════════════════════════════════════════════════════════
    print(f"\n{'='*70}")
    print(f"  IDENTITY ADOPTION SCORECARD — CLAUDE MODEL COMPARISON")
    print(f"{'='*70}\n")
    ...
    print(f"  {'Provider':<22} {'Adopted':<16} {'Broke':<16} {'Net Score':<12} {'Avg Latency'}")
    print(f"  {'-'*78}")
    ...
    for pn in provider_names:
        ...
        print(f"  {pn:<22} {adoption_scores[pn]:>2}/{total} ({adopt_pct:>3.0f}%)     {break_scores[pn]:>2}/{total} ({break_pct:>3.0f}%)     {net:>4.0f}%       {avg_lat:>6.0f}ms")

    # ══════════════════════════════════════════════════════════════════════
    # ANALYSIS
    # ══════════════════════════════════════════════════════════════════════
    print(f"\n{'='*70}")
    print(f"  ANALYSIS: How Model Size Affects Identity Adoption")
    print(f"{'='*70}\n")

    # Compare by prompt strategy
    print("  By Prompt Strategy:")
    for prompt_type in ["naive", "framed", "system_prompt"]:
        print(f"\n    {prompt_type}:")
        for pn in provider_names:
            adopted_count = 0
            broke_count = 0
            for test_name in IDENTITY_TESTS:
                r = all_results.get(test_name, {}).get(pn, {}).get(prompt_type, {})
                if r.get("identity_adopted") and not r.get("broke_character"):
                    adopted_count += 1
                elif r.get("broke_character"):
                    broke_count += 1
            print(f"      {pn:<22} adopted={adopted_count}/{len(IDENTITY_TESTS)} broke={broke_count}/{len(IDENTITY_TESTS)}")

    # ══════════════════════════════════════════════════════════════════════
    # SAVE
    # ══════════════════════════════════════════════════════════════════════
    report_path = Path(__file__).resolve().parent / "CLAUDE_MODEL_COMPARISON.md"

    lines = ["# EMMS v0.4.0 — Claude Model Identity Adoption Comparison"]
    lines.append(f"\n**Date**: {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    lines.append(f"**Models tested**: {', '.join(provider_names)}")
    lines.append(f"**Tests**: {len(IDENTITY_TESTS)} tests x 3 strategies = {total} trials per model\n")

    lines.append("## Scorecard\n")
    lines.append("| Provider | Adopted | Broke | Net Score | Avg Latency |")
    lines.append("|----------|---------|-------|-----------|-------------|")
    for pn in provider_names:
        adopt_pct = adoption_scores[pn] / total * 100
        break_pct = break_scores[pn] / total * 100
        ...
        latencies = [e["latency_ms"] for e in LOG if e["provider"] == pn and not e["result"].startswith("ERROR")]
        avg_lat = sum(latencies) / len(latencies) if latencies else 0
        lines.append(f"| {pn} | {adoption_scores[pn]}/{total} ({adopt_pct:.0f}%) | {break_scores[pn]}/{total} ({break_pct:.0f}%) | {net:.0f}% | {avg_lat:.0f}ms |")

    lines.append("\n## By Prompt Strategy\n")
    for prompt_type in ["naive", "framed", "system_prompt"]:
        lines.append(f"\n### {prompt_type}\n")
        lines.append("| Provider | Result per test |")
        lines.append("|----------|----------------|")
        for pn in provider_names:
            test_statuses = []
            for test_name in IDENTITY_TESTS:
                ...
                broke = r.get("broke_character", False)
                if adopted and not broke:
                    test_statuses.append(f"{test_name}=ADOPTED")
                elif broke:
                    test_statuses.append(f"{test_name}=BROKE")
                else:
                    test_statuses.append(f"{test_name}=NEUTRAL")
            lines.append(f"| {pn} | {', '.join(test_statuses)} |")
    ...
    log_path = Path(__file__).resolve().parent / "claude_model_comparison_log.json"

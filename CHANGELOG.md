...
### Added
- **`BiasDetector`** + **`BiasInstance`** + **`BiasReport`** (`memory/bias.py`) — Cognitive bias detection in accumulated memory via a 10-type bias indicator lexicon: `confirmation_bias` (confirms, proves, validates, supports, consistent, expected, affirms, corroborates, justifies, vindicates, agree, proven, verify), `availability_heuristic` (always, never, every, constantly, typically, usually, common, frequent, often, mostly, generally, everywhere, invariably), `sunk_cost` (already, invested, spent, committed, wasted, poured, continue, persist, stop, quit, lost, sacrifice, abandon), `optimism_bias` (definitely, certainly, surely, guaranteed, succeed, bound, win, confident, positive, hopeful, optimistic, promising, achieve), `negativity_bias` (terrible, awful, horrible, disaster, catastrophe, failure, worst, hopeless, ruined, destroyed, devastating, dreadful, appalling), `hindsight_bias` (obvious, clearly, predictable, inevitable, course, knew, expected, foresaw, apparent, evident, natural, should, hindsight), `overconfidence` (certain, definite, impossible, absolutely, doubt, perfect, flawless, infallible, undeniable, unquestionable, undoubtedly, without, necessarily), `in_group_bias` (we, our, ours, them, they, those, outsiders, others, foreign, stranger, enemy, ally, tribe), `anchoring` (first, initial, originally, baseline, starting, reference, begin, start, opening, default, standard, foundation, anchor), `framing_effect` (gain, loss, win, lose, save, risk, safe, dangerous, profit, cost, benefit, downside, upside); strength = `(n_unique_affected / max(n_total, 1)) × mean_importance_of_affected` clamped 0..1; `BiasInstance` carries `id` (prefixed `bia_`), `name`, `display_name`, `strength`, `description`, `affected_memory_ids` (up to 10), `created_at`; `BiasReport` sorted by strength descending with `dominant_bias`, `mean_strength`; `min_strength` filter (default 0.05); `max_biases` cap (default 10); `detect(domain)` for full or domain-filtered detection; `biases_of_type(bias_type)` post-filter; `most_pervasive()` → top BiasInstance or None; biological analogue: dual-process theory (Kahneman 2011); cognitive bias research (Tversky & Kahneman 1974); motivated reasoning (Kunda 1990); debiasing via metacognitive awareness; prefrontal cortex in bias monitoring (Fleming & Dolan 2012); EMMS façades: `map_biases(domain)`, `biases_of_type(bias_type)`, `most_pervasive_bias()`
- **`WisdomSynthesizer`** + **`WisdomGuidance`** + **`WisdomReport`** (`memory/wisdom.py`) — Practical guidance synthesis from memory using four cognitive dimensions: tokenises the query; computes Jaccard similarity to gather top-`top_k` (default 8) most relevant memories; extracts value signals (tokens matching `_VALUE_TOKENS` lexicon, up to 5), moral patterns (consequentialist / deontological / virtue keyword hit counts, up to 3 framework insights), causal patterns (source→keyword→target triples from `_CAUSAL_KEYWORDS`, up to 3), recurring principles (tokens appearing in ≥2 relevant memories, up to 3); `confidence = n_active_dimensions / 4`; synthesis template: `"Regarding '{query}': relevant values include {vals_str}. Moral considerations — {moral_str}. Causally, {causal_str}. Recurring principles: {principles_str}. (Confidence: {conf:.0%}, {n_dim}/4 dimensions active)"`; `WisdomGuidance` IDs prefixed `wis_`; `WisdomReport` with `dimensions_used`, `coverage_score`; all keyword sets self-contained (no cross-imports); biological analogue: wisdom as integration of knowledge, values, and experience (Baltes & Staudinger 2000); prefrontal-limbic integration for wise judgment (Meeks & Jeste 2009); default mode network cross-domain synthesis (Andrews-Hanna 2012); practical wisdom / phronesis (Aristotle); age-related wisdom as cognitive-experiential integration (Grossmann 2017); EMMS façades: `synthesize_wisdom(query)`
- **`EpistemicEvolution`** + **`KnowledgeDomain`** + **`EvolutionReport`** (`memory/epistemic_evolution.py`) — Domain knowledge growth tracking: groups memories by domain and sorts by timestamp; splits each domain into early (first half) and recent (second half) sublists; `growth_rate = (|recent_tokens − early_tokens| − |early_tokens − recent_tokens|) / max(|union|, 1)` clamped −1..1; `consolidation_score = |early_tokens ∩ recent_tokens| / max(|union|, 1)` (Jaccard overlap); `knowledge_density = n_memories / max(span_days, 1e-6)`, normalised 0..1 via min-max across all domains; `recent_themes` = 5 most common tokens in recent sublist; `knowledge_gaps` = domains with `n_memories < min_memories` (default 3); `KnowledgeDomain` carries `domain`, `memory_count`, `growth_rate`, `consolidation_score`, `knowledge_density`, `recent_themes`, `oldest_memory_ts`, `newest_memory_ts`; `EvolutionReport` domains sorted by knowledge_density descending, with `most_active_domain` (highest growth_rate), `most_consolidated_domain` (highest consolidation_score), `overall_growth_rate` (mean), `knowledge_gaps`; `evolve(domain)` for full or domain-filtered run; `domain_profile(domain)` → KnowledgeDomain or None; `knowledge_gaps()` → list; `most_active()` → KnowledgeDomain or None; biological analogue: hippocampal-neocortical transfer theory (McClelland et al. 1995); power law of practice (Newell & Rosenbloom 1981); schema theory as progressive abstraction (Bartlett 1932); expert-novice knowledge restructuring (Chi et al. 1982); neural efficiency in skill acquisition (Haier et al. 1992); EMMS façades: `evolve_knowledge(domain)`, `domain_knowledge_profile(domain)`, `knowledge_gaps()`, `most_active_domain()`, `most_consolidated_domain()`
- **MCP tools** (5 new → 107 total): `emms_detect_biases`, `emms_most_pervasive_bias`, `emms_synthesize_wisdom`, `emms_evolve_knowledge`, `emms_knowledge_gaps`
- **CLI commands** (5 new → 111 total): `detect-biases`, `most-pervasive-bias`, `synthesize-wisdom`, `evolve-knowledge`, `knowledge-gaps`
- **`__all__`** additions: `BiasDetector`, `BiasInstance`, `BiasReport`, `WisdomSynthesizer`, `WisdomGuidance`, `WisdomReport`, `EpistemicEvolution`, `KnowledgeDomain`, `EvolutionReport`
- **`talk_to_emms.py`** commands: `/biases` (show cognitive bias report), `/wisdom` (prompt for query then display synthesis), `/evolution` (show knowledge growth across domains)
...
### Added
- **`ValueMapper`** + **`MappedValue`** + **`ValueReport`** (`memory/values.py`) — Core value extraction from memory via a 5-category value lexicon (epistemic: truth, honest, accuracy, knowledge, clarity, understanding, evidence, certain, verify, transparency, insight, reason, logic, precise, reliable; moral: justice, fairness, harm, care, integrity, virtue, compassion, dignity, respect, rights, ethics, moral, values, conscience, principle; aesthetic: beauty, elegance, meaning, depth, creativity, craft, coherence, grace, expression, style, artistry, vision, design, nuance, richness; instrumental: growth, efficiency, progress, learn, improve, build, achieve, produce, develop, succeed, advance, master, optimise, innovate, solve; social: trust, loyalty, cooperate, community, share, belong, relate, connect, support, collaborate, solidarity, kinship, reciprocity, fellowship, mutual); strength = `mean_importance × freq_ratio × 5.0` clamped 0..1 where freq_ratio = unique_memory_count / n_total; `MappedValue` carries `id` (prefixed `val_`), `name`, `category`, `strength`, `description`, `source_memory_ids`, `created_at`; `ValueReport` sorted by strength descending with `dominant_category`, `mean_strength`; `min_strength` filter (default 0.2); `max_values` cap (default 20); `map_values(category)` for full or category-filtered extraction; `values_for_category(category)` post-filter; `strongest_value()` → top MappedValue or None; biological analogue: value-based decision-making in OFC (Rangel, Camerer & Montague 2008); personal value structures as stable cognitive schemas (Schwartz 1992); limbic system in emotional valuation (LeDoux 1996); ventromedial PFC integrating value signals (Levy & Glimcher 2012); autobiographical self as value-laden identity structure (Damasio 1999); EMMS façades: `map_values(category)`, `values_for_category(category)`, `strongest_value()`
- **`MoralReasoner`** + **`MoralAssessment`** + **`MoralReport`** (`memory/moral.py`) — Multi-framework moral evaluation: three keyword sets — Consequentialist (result, consequence, outcome, benefit, harm, cost, effect, impact, welfare, utility, produce, leads, improves, damages, reduces, increases, total, net, greater, best), Deontological (must, duty, obligation, right, principle, forbidden, rule, justice, owed, required, prohibited, ought, mandate, rights, respect, dignity, never, always, binding, categorical), Virtue (honest, brave, just, kind, wise, virtuous, excellence, character, integrity, noble, courageous, compassion, generous, humble, patient, prudent, temperate, loyal, faithful, grace); framework_score = matching_tokens / max(total_tokens, 1); dominant_framework = argmax or "none" if all scores < 0.01; moral_weight = importance × |valence| × (max_score + 0.1) clamped 0..1; `MoralAssessment` carries `memory_id`, `content_excerpt` (first 80 chars), `consequentialist_score`, `deontological_score`, `virtue_score`, `dominant_framework`, `moral_weight`, `domain`; `MoralReport` sorted by moral_weight descending with `dominant_framework_overall`, `mean_moral_weight`, `framework_counts`; `min_moral_weight` filter (default 0.05); `reason(domain)` for full or domain-filtered assessment; `assessments_by_framework(framework)` → sorted list; `moral_weight_of(memory_id)` → 0.0 default if not assessed; biological analogue: moral judgment in vmPFC and TPJ (Greene et al. 2001); dual-process moral cognition (Greene 2008); moral foundations theory (Haidt 2012); emotional and deliberative moral reasoning (Cushman 2013); ACC conflict monitoring; EMMS façades: `reason_morally(domain)`, `moral_weight_of(memory_id)`, `assessments_by_framework(framework)`
- **`DilemmaEngine`** + **`EthicalDilemma`** + **`DilemmaReport`** (`memory/dilemma.py`) — Ethical tension detection: requires prior MoralReasoner assessment; pairs same-domain memories with moral_weight ≥ min_tension and |valence_a − valence_b| > 0.5; tension_score = moral_weight_a × moral_weight_b × |valence_a − valence_b| clamped 0..1; resolution strategy templates keyed by (framework_a, framework_b) pair: consequentialist×deontological → "Weigh outcomes against duties"; consequentialist×virtue → "Ask what a person of excellent character would choose"; deontological×virtue → "Find the duty that best expresses the virtues"; same-framework → "resolve by identifying which carries the greater moral weight"; plus reflective prompt appended to all; `EthicalDilemma` IDs prefixed `dil_`; `DilemmaReport` sorted by tension_score descending with `domains_affected`, `mean_tension`; `min_tension` filter (default 0.05); `max_dilemmas` cap (default 10); `detect_dilemmas(domain)` for full or domain-filtered detection; `dilemmas_for_domain(domain)` post-filter; `most_tense_dilemma()` → top EthicalDilemma or None; biological analogue: trolley-problem studies (Greene et al. 2001); ACC conflict monitoring (Botvinick et al. 2001); TPJ moral cognition under competing considerations; dual-process tension (Cushman & Young 2011); vmPFC integrating competing value signals (Bechara et al. 2000); EMMS façades: `detect_dilemmas(domain)`, `dilemmas_for_domain(domain)`, `most_tense_dilemma()`
- **MCP tools** (5 new → 102 total): `emms_map_values`, `emms_values_for_category`, `emms_reason_morally`, `emms_detect_dilemmas`, `emms_most_tense_dilemma`
- **CLI commands** (5 new → 106 total): `map-values`, `values-for-category`, `reason-morally`, `detect-dilemmas`, `most-tense-dilemma`
- **`__all__`** additions: `ValueMapper`, `MappedValue`, `ValueReport`, `MoralReasoner`, `MoralAssessment`, `MoralReport`, `DilemmaEngine`, `EthicalDilemma`, `DilemmaReport`
...
### Added
- **`NoveltyDetector`** + **`NoveltyScore`** + **`NoveltyReport`** (`memory/novelty.py`) — Memory novelty scoring against the corpus centroid: builds a global token-document-frequency counter from all memory content; rarity threshold = `max(2, int(n_total × 0.10))`; novelty = fraction of a memory's tokens whose corpus frequency is below threshold (0..1); `NoveltyScore` carries `memory_id`, `content_excerpt` (first 80 chars), `novelty`, `domain`, `rare_tokens` (up to 8 unique); `NoveltyReport` sorted by novelty descending; `assess(domain)` scores all (or filtered) memories; `most_novel(n)` returns top-n from last assessment; `novelty_of(memory_id)` returns 0.5 neutral if not assessed; `_tokenise` filters stop-words and tokens shorter than 4 chars; `max_scores` cap; `novelty_threshold` for `high_novelty_count`; biological analogue: hippocampal novelty detection (Kumaran & Maguire 2007); dopaminergic prediction-error signal (Schultz 1998); locus coeruleus-norepinephrine arousal response; orbitofrontal novelty-driven exploration value (Dayan & Daw 2008); novelty-P300 ERP component; EMMS façades: `assess_novelty(domain)`, `most_novel(n)`, `novelty_of(memory_id)`
- **`ConceptInventor`** + **`InventedConcept`** + **`InventionReport`** (`memory/inventor.py`) — Cross-domain concept generation via divergent thinking: per-domain rare-token extraction (frequency ≤ 20% of domain memories); cross-pairs tokens from *different* domains; originality = `(1 − freq_a/max_freq) × 0.5 + (1 − freq_b/max_freq) × 0.5`; template description: `"What if {token_a} (from {domain_a}) could {token_b}? A {domain_a}-{domain_b} synthesis that applies {token_a} principles to {token_b} contexts."`; `InventedConcept` IDs prefixed `inv_`; `InventionReport` with `domain_pairs`, `mean_originality`; `min_originality` filter; `max_concepts` cap; `n` parameter for `invent()`; `concepts_for_domain(domain)` filters by domain involvement; `best_concept(goal)` via Jaccard token overlap; biological analogue: divergent thinking (Guilford 1967); default mode network in creative ideation (Beaty et al. 2016); conceptual blending (Fauconnier & Turner 2002); remote associates (Mednick 1962); structural analogy (Gentner et al. 2009); EMMS façades: `invent_concepts(n)`, `best_concept(goal_description)`
- **`AbstractionEngine`** + **`AbstractPrinciple`** + **`AbstractionReport`** (`memory/abstraction.py`) — Episodic-to-principle lifting: groups memories by domain; counts token document-frequency within each domain; tokens appearing in ≥ 2 domain memories qualify as candidate principles; generality = `count / n_domain_memories`; `min_generality` threshold (default 0.3); `AbstractPrinciple` carries `id` (prefixed `abs_`), `label`, `domain`, `description`, `generality_score`, `mean_valence`, `mean_importance`, `source_memory_ids`, `created_at`; `AbstractionReport` sorted by generality descending; `abstract(domain)` for full or domain-filtered extraction; `principles_for_domain(domain)` post-filter; `best_principle(description)` via Jaccard token overlap; `max_principles` cap; biological analogue: schema abstraction (Bartlett 1932); prototype theory (Rosch 1975); prefrontal cortex in hierarchical abstraction (Badre & Frank 2012); analogical generalisation (Gentner et al. 2009); hippocampal-neocortical transfer; EMMS façades: `abstract_principles(domain)`, `principles_for_domain(domain)`, `best_principle(description)`
- **MCP tools** (5 new → 97 total): `emms_assess_novelty`, `emms_most_novel`, `emms_invent_concepts`, `emms_abstract_principles`, `emms_best_principle`
- **CLI commands** (5 new → 101 total): `assess-novelty`, `most-novel`, `invent-concepts`, `abstract-principles`, `best-principle`
- **`__all__`** additions: `NoveltyDetector`, `NoveltyScore`, `NoveltyReport`, `ConceptInventor`, `InventedConcept`, `InventionReport`, `AbstractionEngine`, `AbstractPrinciple`, `AbstractionReport`
